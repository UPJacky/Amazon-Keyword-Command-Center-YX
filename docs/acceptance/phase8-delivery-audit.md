...
- 用户已提交带地址栏的登录页与工具页截图；其报告截图实际打开了未携带 `task/run` 的空报告或模块失败页，不能作为真实报告证据。正确报告地址必须由工具页已完成任务行的“查看报告”生成，并带 `?task=...&run=...`。
...
| 三张带地址栏截图、乙线匿名 API 拒绝截图 | 登录页与工具页地址栏截图已收到；报告截图因未携带 `task/run` 而无真实报告内容，需从任务行“查看报告”重新截取。乙线匿名 API 已实测 401，截图仍按交作业要求保留人工项。 |
