...
日期：2026-09-04。本文区分本地可重跑证据与线上真实复核；线上发布、真实浏览器和无痕窗口证据不以本地模拟冒充。
...
- `frontend/tool/index.html`、`frontend/report/index.html` 与根登录入口是独立 HTML 地址；Pages 构建审计固定核对 57 个文件、无断链、无网络调用，生产候选包为 49 个文件。
...
- 线上 `index.html`、`tool/index.html`、`tool/tool.js`、`report/index.html`、`report.js`、`styles.css` 均返回 200；生产候选包已发布，根入口、`tool/`、`report/` 和 Auth/REST Gateway 预检均已复验。
- 用户已补交从工具页已完成任务行“查看报告”进入的真实报告页截图 `docs/acceptance/查看报告1.jpg`；截图带浏览器地址栏，地址包含 `task/run`，页面显示 91 个搜索词、广告花费 `$5,123.10`、订单 `1,578`、CTR `0.7%`、广告范围缺失声明和“对账通过，差值 0”。此前未携带 `task/run` 的空报告/模块失败截图不再作为证据。
...
| 线上 Pages 精确地址/重发布 | 生产候选包已发布；根入口、`/tool/`、`/report/` 均 HTTPS 200，Gateway 精确 Origin 与 Auth/REST 已复验。 |
| 未登录窗口打开报告页 | 通过；BrowserSkill 隔离 Agent Window 清除演示会话后直接打开 `/report/`，自动跳回 `/index.html`，报告内容不可见。 |
| 三张带地址栏截图、乙线匿名 API 拒绝截图 | 登录页、工具页和 `docs/acceptance/查看报告1.jpg` 三张地址栏截图已齐；报告截图含真实 `task/run`、在线报告内容和差值 0。乙线匿名 API 已实测 HTTP 401 `AUTH_REQUIRED`；如作业明确要求图片形式，可另存截图，但不阻断生产链路 Gate。 |
