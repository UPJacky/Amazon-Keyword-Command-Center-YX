...
每次任务最多选择前 10 个去重关键词并发出一次请求。调用前同时预留 1 次调用和 1 credit；返回的 `cost_credits` 必须是非负整数。未知费用、超出预留、传输异常、HTTP/MCP/业务错误会锁住共享预算，防止无法对账时继续花费。预算是单进程边界，不能冒充分布式账户配额。生产组合还必须注入一个跨 Provider 的 `ProviderAttemptBudget`；它只统计请求尝试次数，重试也计数，缓存命中不计数，不代表统一费用。
...
```text
python scripts/run_production_worker.py --confirm-live --xiyou-keywords 10 --max-provider-calls 10 --max-provider-credits 10 --max-total-provider-attempts 30
```

示例中的 `30` 只是本次部署者批准的进程级总尝试硬上限，不是平台默认额度；必须结合当前任务规模、重试策略和其他 Provider 请求量重新批准。`--xiyou-keywords` 与 `--ad-only` 互斥，Xiyou 的调用、credit 和跨 Provider 总尝试三个预算都必填且必须为正整数；未显式启用时 Worker 继续生成 `ad_only` 报告。真实调用、账户总额度、其他西柚工具、完整自然位/竞品/图片模块是独立外部或业务 Gate，不能由本 fake 测试关闭。
